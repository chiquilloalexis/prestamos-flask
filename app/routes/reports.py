...
def _rango_por_filtro(filtro, mes=None, desde=None, hasta=None):
    hoy = date.today()
    if filtro == "dia":
        return hoy, hoy
    if filtro == "semana":
    ...
    if filtro == "mes_especifico":
        # "mes" llega como texto "AAAA-MM" desde un <input type="month">.
        # Si por algún motivo no llega bien formado, no rompemos el
        # reporte: caemos de vuelta al mes actual.
        if mes:
            try:
        ...
        return hoy.replace(day=1), hoy
    if filtro == "rango":
        # Rango de fechas totalmente libre, elegido a mano.
        try:
            inicio = datetime.strptime(desde, "%Y-%m-%d").date() if desde else hoy.replace(day=1)
            fin = datetime.strptime(hasta, "%Y-%m-%d").date() if hasta else hoy
            if inicio > fin:
...
def _calcular_reporte(filtro, cliente_id=None, mes=None, desde=None, hasta=None):
    inicio, fin = _rango_por_filtro(filtro, mes=mes, desde=desde, hasta=hasta)
...
def index():
    ...
    reporte = _calcular_reporte(filtro, cliente_id, mes=mes, desde=desde, hasta=hasta)

    return render_template(
        "reportes.html", reporte=reporte, filtro=filtro,
        clientes=clientes, cliente_id=cliente_id,
        mes=mes, desde=desde, hasta=hasta,
    )
