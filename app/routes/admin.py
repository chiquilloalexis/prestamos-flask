...
def prestamo_nuevo(cliente_id):
    ...
    pero solo si ya terminó de pagar el préstamo que tenía activo."""
    cliente = Cliente.query.get_or_404(cliente_id)

    prestamo_actual = cliente.prestamos[-1] if cliente.prestamos else None
    if prestamo_actual and not prestamo_actual.esta_saldado:
        flash(
            "Este cliente todavía tiene un préstamo activo. No se puede "
            "agregar un crédito nuevo hasta que termine de pagar el actual.",
            "danger",
        )
        return redirect(url_for("admin.cliente_detalle", cliente_id=cliente.id))
    ...
    db.session.add(prestamo)
    db.session.commit()
    flash("Nuevo crédito registrado.", "success")
    return redirect(url_for("admin.cliente_detalle", cliente_id=cliente.id))
