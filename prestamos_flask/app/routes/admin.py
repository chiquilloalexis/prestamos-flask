...
def dashboard():
    ...
    en_mora = [p for p in prestamos if p.estado == Prestamo.ESTADO_MORA]
...
def _serie_ultimos_meses(n):
    ...
    for (y, m) in meses:
        pagos = Pago.query.filter(
            func.year(Pago.fecha) == y, func.month(Pago.fecha) == m
        ).all()
...
def _serie_clientes_nuevos(n):
    ...
    for (y, m) in meses:
        c = Cliente.query.filter(
            func.year(Cliente.created_at) == y, func.month(Cliente.created_at) == m
        ).count()
        counts.append(c)
