...
class Prestamo(db.Model):
    ...
    def dias_desde_ultimo_pago(self, dias_mora=2):
        ultimo = self.pagos[0].fecha if self.pagos else self.fecha_prestamo
        return (date.today() - ultimo).days

    def actualizar_estado(self, dias_mora=2):
        """Recalcula y guarda el estado del préstamo. Se llama tras cada pago
        y también puede correrse en batch (ver comando CLI actualizar-mora)."""
        if self.esta_saldado:
            self.estado = self.ESTADO_CANCELADO
        elif self.dias_desde_ultimo_pago(dias_mora) >= dias_mora:
            self.estado = self.ESTADO_MORA
        else:
...
class Configuracion(db.Model):
    ...
    (nombre del negocio, días de mora, etc.)."""
